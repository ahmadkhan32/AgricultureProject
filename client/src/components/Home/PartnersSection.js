    ...
    <section className="py-8 sm:py-12 md:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-3 sm:mb-4">Nos Partenaires</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6 lg:gap-8">
          {partners.map((partner, index) => {
            ...
            return (
              <div
                key={index}
                className="group flex flex-col items-center justify-center p-4 sm:p-6 bg-bg-light rounded-lg border border-gray-200 hover:border-accent-300 hover:shadow-md transition-all duration-300 hover:scale-105"
              >
                {partner.logo ? (
                  <img
                    src={partner.logo}
                ...
                <div
                  className={`${partner.logo ? 'hidden' : 'flex'} w-12 h-12 sm:w-16 sm:h-16 bg-primary-100 rounded-lg items-center justify-center mb-2 sm:mb-3 group-hover:bg-accent-100 transition-colors`}
                >
                  <Icon className="w-6 h-6 sm:w-8 sm:h-8 text-primary-500 group-hover:text-accent-500" />
                </div>
